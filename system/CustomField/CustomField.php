...
abstract class _CustomField extends \IPS\Node\Model
{
	...
	{		
		...
		$toggles = array(
			...
			'Editor'		=> array( 'pf_not_null', "{$form->id}_header_pfield_displayoptions", 'pf_format' ),
		...
		switch ( $this->type )
		{
			...
			case 'CheckboxSet':
				...
				if ( $this->multiple )
				{
					...
					{
						$value = explode( ',', htmlspecialchars( $value, \IPS\HTMLENTITIES, 'UTF-8', FALSE ) );
					}
	...
	public function displayValue( $value=NULL )
	{
		switch ( $this->type )
		{
			...
			case 'Select':
				...
				if ( $this->multiple )
				{
					return implode( '<br>', explode( ',', htmlspecialchars( $value, \IPS\HTMLENTITIES, 'UTF-8', FALSE ) ) );
				}
				else
				{
					return htmlspecialchars( \IPS\Lang::wordbreak( $value ), \IPS\HTMLENTITIES, 'UTF-8', FALSE );
				}
			...
			case 'TextArea':
				return \IPS\Lang::wordbreak( nl2br( htmlspecialchars( $value, \IPS\HTMLENTITIES, 'UTF-8', FALSE ) ) );
				
			case 'Url':
				$munged = ( $value ) ? \IPS\Http\Url::external( $value )->makeSafeForAcp() : NULL;

				/* skip the default template if we have a custom output display value */
				if ( $value AND isset( $this->format ) AND $this->format != '' )
				{
					return $munged;
				}
			...
			default:
				return \IPS\Lang::wordbreak( htmlspecialchars( $value, \IPS\HTMLENTITIES, 'UTF-8', FALSE ) );
		}
	}
}