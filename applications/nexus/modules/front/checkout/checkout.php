...
class _checkout extends \IPS\Dispatcher\Controller
{
	...
	public function _customer()
	{
		...
		if ( $this->needsToLogin )
		{
			...
			\IPS\Member::loggedIn()->language()->words[ "reg_agreed_terms" ] = sprintf( \IPS\Member::loggedIn()->language()->get("reg_agreed_terms"), \IPS\Http\Url::internal( 'app=core&module=system&controller=terms', 'front', 'terms', array(), PROTOCOL_RELATIVE ) );
			
			/* Build the appropriate links for registration terms & privacy policy */
			if ( \IPS\Settings::i()->privacy_type == "internal" )
			{
				\IPS\Member::loggedIn()->language()->words[ "reg_agreed_terms" ] .= sprintf( \IPS\Member::loggedIn()->language()->get("reg_privacy_link"), \IPS\Http\Url::internal( 'app=core&module=system&controller=privacy', 'front', 'privacy', array(), PROTOCOL_RELATIVE ), 'data-ipsDialog data-ipsDialog-size="wide" data-ipsDialog-title="' . \IPS\Member::loggedIn()->language()->get("privacy") . '"' );
			}
	...
	public function _pay( $data )
	{
		...
		/* Nothing to pay? */
		if ( $amountToPay->amount < 0 )
		{
			\IPS\Output::i()->error( 'err_no_methods', '5X196/8', 500, '' );
		}
		elseif ( $amountToPay->amount === (float) 0 )
		{
		...
		{
			$form->hiddenValues['split'] = $amountToPay->amount;
		}
		...
		if ( $values !== FALSE )
		{
			...
			if ( isset( $values['payment_method'] ) and $values['payment_method'] == 0 )
			{
				...
				$credits[ $this->invoice->currency ]->amount -= abs( $transaction->amount->amount );
